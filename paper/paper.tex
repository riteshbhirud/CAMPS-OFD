...
The bound can be understood more deeply through the block structure.
...
By the linearity of the symplectic representation~\cite{Dehaene2003}, the row space of $z^{(k)}$ is the image of the row space of $z^{(1)}$ under the cumulative inter-block Clifford transformation.
Specifically, let $D_k$ denote the combined Clifford between blocks 1 and $k$, with symplectic matrix $S_{D_k}$.
Then $\mathrm{rowspace}(z^{(k)}) = L_k(\mathrm{rowspace}(z^{(1)}))$ where $L_k$ is the invertible $\mathbb{F}_2$-linear map on $\mathbb{F}_2^N$ determined by the $G$-block of $S_{D_k}$.
The total rank is
\begin{equation}
\mathrm{rank}(z) = \dim\bigl(\textstyle\sum_{k=1}^r L_k(V_1)\bigr),
\end{equation}
where $V_1 = \mathrm{rowspace}(z^{(1)}) \subseteq \mathbb{F}_2^N$ has $\dim(V_1) = \mathrm{rank}(z^{(1)}) \le \min(g,N)$.
Each subspace $L_k(V_1)$ has the same dimension as $V_1$ (since $L_k$ is invertible), but the sum has dimension at most $N$.
When $g \ge N$, we typically have $\mathrm{rank}(z^{(1)}) = N$, so that $V_1 = \mathbb{F}_2^N$ and consequently $\mathrm{rank}(z) = N$ \emph{independently of $r$}; in this regime, every non-Clifford gate beyond the first $N$ contributes only to nullity.
...
We employ a hybrid disentangling strategy: for each non-Clifford gate, OFD is attempted first following Liu and Clark's criterion~\cite{Liu2024}; on failure, we fall back to OBD following Qian \emph{et al.}~\cite{Qian2024}, searching over all 720 equivalence classes of two-qubit Clifford gates (up to phases) to minimize the second R\'enyi entropy, using their efficient $O(\chi^3 + |\mathrm{Cl}_2|)$ evaluation method.
...
Our simulations employ OBD fallback (Section~\ref{sec:validation}), which successfully controls bond dimension for structured circuits at $N \le 16$: OBD searches over 720 two-qubit Clifford gate classes to minimize the second R\'enyi entropy at each bond, actively reducing entanglement at every step.
